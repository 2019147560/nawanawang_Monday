...
# Claude 응답의 max_tokens는 "목표 글자 수"가 아니라 하드 캡입니다.
# 한글은 토큰당 대략 0.5~0.7자라서, 목표 글자 수(BLOG_LENGTH)보다 여유 있게
...
# ── Anthropic API 호출 ────────────────────────────────────────────────────────
def call_anthropic(prompt: str, api_key: str, max_tokens: int = MAX_OUTPUT_TOKENS) -> str:
    payload = json.dumps({
        "model": MODEL,
...
def main():
    ...
    print(f"   목표 길이: {TARGET_LENGTH}자 (토큰 예산: {MAX_OUTPUT_TOKENS} tokens)")
